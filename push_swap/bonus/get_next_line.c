...
char	*get_next_line(int fd)
{
	...
	if (node == NULL)
	{
		node = ft_lstnew_gnl(&head, head, fd);
		if (node == NULL)
			return (NULL);
	}
	...
	node -> buf = ft_substr_gnl(node -> buf);
...
t_list_gnl	*ft_lstnew_gnl(t_list_gnl **head, t_list_gnl *lst, int fd)
{
	t_list_gnl	*node;
...
char	*read_file(t_list_gnl *node, int fd)
{
	...
	while (1)
	{
		...
		buf[ret] = '\0';
		node -> buf = ft_strjoin_gnl(node -> buf, buf);
		if (ret == 0 || node -> buf == NULL || ft_strchr_gnl(node -> buf, '\n'))
			break ;
	}
	free(buf);
	return (node -> buf);
