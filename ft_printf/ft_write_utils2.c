...
char	*ptr_to_str(uintptr_t num, char *charset)
{
	...
	if (ft_numlen(num, ft_strlen(charset)) == 0)
		return (ft_strdup("0"));
	len = ft_numlen(num, ft_strlen(charset));
...
char	*add_prefix(t_conv *conv, intptr_t n, char *charset)
{
	...
	if (n < 0)
		n *= -1;
	if (prefix_hex(conv, ft_strlen(charset), n, &buf) < 0)
	{
		free(buf);
		return (NULL);
	}
	if (prefix_prec(conv, ft_strlen(charset), n, &buf) < 0)
	{
		free(buf);
		return (NULL);
	}
