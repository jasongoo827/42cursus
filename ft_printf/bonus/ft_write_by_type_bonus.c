...
int	type_str(t_conv *conv, char *str)
{
	...
	if (str == NULL)
		str = "(null)";
	maxlen = len_by_prec(conv, ft_strlen(str));
	ret = fill_str(conv, str, maxlen);
	if (ret == NULL)
		return (-1);
	res = (int)write(1, ret, ft_strlen(ret));
	if (res < 0)
	{
		free(ret);
		return (-1);
...
int	type_ptr(t_conv *conv, void *ptr)
{
	...
	ret[3] = add_padding(conv, ft_strlen(ret[1]) + ft_strlen(ret[2]));
	...
	res = (int)write(1, ret[0], ft_strlen(ret[0]));
	ft_abort_write(ret);
	if (res < 0)
		return (-1);
	return (res);
...
int	type_dec(t_conv *conv, int n, char *charset)
{
	...
	ret[2] = num_to_str(conv, n, ft_strlen(charset), charset);
	ret[3] = add_padding(conv, ft_strlen(ret[1]) + ft_strlen(ret[2]));
	ret[0] = merge_str(conv, ret);
	if (ret[0] == NULL)
	{
